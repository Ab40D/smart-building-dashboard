...
function StatusBadge({ status }: { status: 'completed' | 'in-progress' }) {
  return status === 'completed' ? (
    <span className="tag">
      <span className="h-1.5 w-1.5 rounded-full bg-signal" aria-hidden /> Completed
    </span>
  ) : (
...
function ArchitectureFlow() {
  ...
  return (
    <div className="border border-white/15 bg-white/[0.02] p-5 md:p-6">
      <p className="flex items-center gap-2 font-mono text-[11px] uppercase tracking-wider2 text-accent">
      ...
      <div className="mt-4 grid gap-2 md:grid-cols-5">
        {steps.map((s, i) => (
          <div key={s.title} className="relative">
            <div className="border border-white/15 bg-ink-900 px-3 py-3">
              <p className="font-mono text-[10px] text-accent">{`0${i + 1}`}</p>
              <p className="mt-1 font-display text-base font-bold uppercase text-white leading-tight">
                {s.title}
              </p>
              <p className="mt-1 text-[11px] text-white/60 leading-snug">{s.sub}</p>
            </div>
            {i < steps.length - 1 && (
      ...
      <p className="mt-4 text-[11px] font-mono uppercase tracking-wider2 text-white/40">
...
export function ProjectsSection() {
  ...
  return (
    <section id="projects" className="scroll-mt-20 py-16 md:py-24 border-t border-paper-200">
      ...
      {/* Featured project */}
      {featuredProject && (
        <article className="group relative border border-ink-900 bg-ink-900 text-white p-6 md:p-8 mb-12 overflow-hidden">
          {/* faint dotted bg */}
          <div
            className="absolute inset-0 opacity-30 bg-dots-dark pointer-events-none"
            aria-hidden
          />
          {/* Subtle scan line */}
          ...
          <div className="relative">
            <div className="flex flex-wrap items-center gap-3 mb-5">
              <span className="bg-accent px-2.5 py-1 text-[11px] font-mono uppercase tracking-wider2 flex items-center gap-2">
            ...
            {featuredProject.github && (
              <div className="mt-7 flex flex-wrap gap-3">
                <a
                  ...
                  className="group/gh inline-flex items-center gap-2 bg-white text-ink-900 px-4 py-2.5 text-[12px] font-mono uppercase tracking-wider2 hover:bg-accent hover:text-white transition-colors"
                >
                  <IconGithub className="h-4 w-4" />
                  View on GitHub
                  <IconExternal className="h-3.5 w-3.5 transition-transform group-hover/gh:translate-x-0.5 group-hover/gh:-translate-y-0.5" />
                </a>
              </div>
            )}
          </div>
      ...
      <div className="grid gap-5 md:grid-cols-2">
        {rest.map((p) => (
          <article
            key={p.name}
            className="group relative flex flex-col border border-paper-200 bg-white p-6 hover:border-ink-900 transition-colors overflow-hidden"
          >
            {/* Top corner accent line on hover */}
            <span
              className="absolute top-0 left-0 h-[2px] w-0 bg-accent transition-all duration-300 group-hover:w-full"
              aria-hidden
            />
      ...
      <div className="mt-10 flex flex-wrap items-center justify-between gap-4 border-t border-paper-200 pt-6">
        ...
        <a
          ...
          target="_blank"
          rel="noreferrer noopener"
          className="group/all inline-flex items-center gap-2 text-[12px] font-mono uppercase tracking-wider2 text-accent hover:text-accent-600"
        >
          View all repositories on GitHub
          <IconExternal className="h-3.5 w-3.5 transition-transform group-hover/all:translate-x-0.5 group-hover/all:-translate-y-0.5" />
        </a>
      </div>
    </section>
  )
